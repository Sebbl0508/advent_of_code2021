...
fn part_02() {
    let bytes = ASSETS_FOLDER.get_file("day02part02.input").unwrap().contents();
    let string = String::from_utf8_lossy(bytes);
    let mut lines = string.lines();
    ...
    println!("02/02: Horizontal speed and depth multiplie: {}", my_sub.horizontal_position * my_sub.depth);
}
